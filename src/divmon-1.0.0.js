...
function exportToXls(xlsName) {
	var a = document.createElement("a");
	var dataType = "data:application/vnd.ms-excel; base64,";
	var table = gI("excel");
	for (let x = 0; x < table.rows.length; x++) {
		let tr = table.rows[x];
		for (let y = 0; y < tr.cells.length; y++) {
			let td = tr.cells[y];
			td.innerHTML = toBinary(td.innerHTML.replace(/\r?\n|\r/, "").replaceAll("\r", "").replaceAll("\n", "").replaceAll("<br>", "").replaceAll("<br/>", ""));
		}
	}
	a.href = dataType + window.btoa(unescape(encodeURIComponent(table.outerHTML)));
...
function toBinary(string) {
	...
	return String.fromCharCode(...new Uint16Array(codeUnits.buffer));
}
